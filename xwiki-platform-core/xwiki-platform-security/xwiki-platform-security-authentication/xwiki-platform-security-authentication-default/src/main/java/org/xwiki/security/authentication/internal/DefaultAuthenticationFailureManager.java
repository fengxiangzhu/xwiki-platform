...
public class DefaultAuthenticationFailureManager implements AuthenticationFailureManager, Initializable, Disposable
{
    ...
    /**
     * Define for which username the security should be skipped: we skip empty values and {@code null} value to avoid
     * any problem with Guest user.
     * @param username the login string to check.
     * @return {@code true} only if the security measure can be skipped.
     */
    private boolean skipLogin(String username)
    {
        return StringUtils.isEmpty(username) || "null".equals(username);
    }
    ...
    public boolean recordAuthenticationFailure(String username, HttpServletRequest request)
    {
        ...
        if (!isAuthenticationSecurityEnabled()) {
            ...
            return false;
        } else if (this.skipLogin(username)) {
            return false;
        }
    ...
    public String getForm(String username, HttpServletRequest request)
    {
        ...
        if (isThresholdReached(username) || isSessionAlreadyFailing(request)) {
            for (AuthenticationFailureStrategy authenticationFailureStrategy : getFailureStrategyList()) {
